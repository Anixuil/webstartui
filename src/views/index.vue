 ...
 * @LastEditors: Anixuil
 * @LastEditTime: 2024-12-29 21:28:23
 * @Description: 首页
-->
<template>
    <div class="views-common-wrapper">
            ...
            <el-table :data="tableData" height="400" style="width: 100%" empty-text="暂无项目" v-loading="loading">
                <el-table-column prop="projectStatus" label="状态" width="110">
                    <template #default="{ row }">
                        <el-tag :type="{ '1': 'success' }[row.projectStatus as string] || 'danger'">
                            {{ row.projectStatus === '1' ? 'RUNNING' : 'STOP' }}
                        </el-tag>
                    </template>
                </el-table-column>
                <el-table-column prop="projectName" label="项目" width="180" >
                    <template #default="{row}">
                        <el-link :underline="false" type="primary" @click="openProjectUrl(row)">{{ row.projectName }}</el-link>
                    </template>
                </el-table-column>
                <el-table-column prop="projectType" label="项目类型" width="100">
                    <template #default="{ row }">
                        <el-tag :type="{ '1': 'success' }[row.projectType as string] || 'primary'">
                ...
                <el-table-column label="操作" width="120" fixed="right">
                    <template #default="{ row }">
                        ...
                        <el-button type="primary" text class="table-button" @click="openDialog('运行日志')">
        ...
        <!-- 弹窗 -->
        <el-dialog class="custom-class" v-model="dialogVisible" :title="dialogTitle" width="75%" destroy-on-close>
            <template #header>
                <div class="title">
                    <span>
                        {{ dialogTitle }}
            ...
            <logInfo v-if="dialogTitle === '运行日志'"></logInfo>
